...
import copy
from best_results_hyperparam_mapping import get_best_hyperparam_mapping
...
prediction_lengths = [192, 336, 720] # 96
models = ["iTransformer", "TimeXer", "PatchTST", "Crossformer", "Autoformer"]

best_configs = get_best_hyperparam_mapping()
        ...
        for dataset_name, dataset_root_path in datasets_path_mapping.items(): # ABLATION FOR BASE DATASETS
            ...
            for config in base_configs: # SEARCHING MODELS AND PREDICTION LENGTHS IN BASE CONFIGS
                if config.model == model and config.forecast.pred_len == pred_len and config.data.name == dataset_name: 
                    ...
                    if dataset_name in best_configs and model in best_configs[dataset_name]:
                        pconstructor = best_configs[dataset_name][model]["pconstructor"]
                        pwindow = best_configs[dataset_name][model]["pwindow"]
                    else:
                        continue
